...
std::vector<uint8_t> Base64Decode(const std::string& input)
{
  ...
  {
    if (std::isspace(c)) continue; // 清洗掉空白字符再解码
...
bool DecompressZlib(const std::vector<uint8_t>& input, std::vector<uint8_t>& output)
{
  ...
  // 标准 zlib 头部优先，不行再尝试 raw deflate
...
std::vector<float> GLLNodes(unsigned int n)
{
  ...
  const unsigned int N = n - 1; // polynomial degree
  ...
  for (unsigned int i = 1; i < n - 1; ++i)
  {
    ...
    for (int iter = 0; iter < maxIter; ++iter)
    {
      ...
      // P''_N via derivative of n/(x^2-1)*(x P_n - P_{n-1})
      const double denom = x * x - 1.0;
...
// --- topology: edges, faces, elements ---
  ...
  auto build_hex_from_faces = [&](const std::array<int, 6>& faceIds, ElemHex& out) -> bool
  {
    ...
    auto face_vertices = [&](int fid) -> std::vector<int>
    {
      ...
      if (it == faceMap.end() || it->second.verts.empty()) return result;
    ...
    // Try to reconstruct the canonical vertex ordering (0-7) via face intersections
    // Face order in HEX tag is assumed to follow Nektar++ convention: {bottom, front, right, back, left, top}
    ...
    if (haveAllFaces)
    {
      ...
      {
        for (int i = 0; i < 8; ++i) out.verts[i] = canon[i];
        for (int e = 0; e < 12; ++e)
        {
  ...
  // --- CURVED data ---
  struct CurvedEdge { unsigned int numPts{0}; std::vector<float3> pts; NodeDistribution nodeType{NodeDistribution::GLL}; };
  struct CurvedFace { unsigned int numPts{0}; char type{'Q'}; std::vector<float3> pts; NodeDistribution nodeType{NodeDistribution::GLL}; };
  ...
  if (!fldText.empty())
  {
    ...
    if (!payload.empty())
    {
      // 先尝试 ASCII，再尝试 base64+zlib（raw deflate 或带 header）
      ...
      if (coeffs.empty())
      {
        ...
        if (!decoded.empty())
        {
          // 尝试 raw deflate / zlib header，两者都失败则当作未压缩二进制
          if (!DecompressZlib(decoded, inflated)) inflated = decoded;
          if (!inflated.empty()) coeff_bytes = std::move(inflated);
        }
      }
      if (!coeff_bytes.empty()) {
          coeffs = DecodeBinaryCoefficients(coeff_bytes);
        }
    }
  }
                                ...
                                uint3& geomModes) -> bool
  {
    ...
    for (int lf = 0; lf < 6; ++lf)
    {
      ...
      std::vector<float3> faceGrid = ResampleQuadFace(cf->second.pts,
                                                      dimU,
                                                      dimV,
                                                      cornersArr,
                                                      cf->second.nodeType,
