...
bool IntersectRayWithFace(const zidingyi::HexElementData& elem,
                          ...
                          int                            maxIter   = 50)
{
    int   fixedAxis = 2;   // 默认 t
    float fixedVal  = -1.f;
    int   uAxis     = 0;   // 自由参数 (u,v) 分别对应 rst[uAxis], rst[vAxis]
    int   vAxis     = 1;

    switch (faceIdx)
    {
    case 0: fixedAxis = 2; fixedVal = -1.f; uAxis = 0; vAxis = 1; break; // t=-1
    case 1: fixedAxis = 2; fixedVal =  1.f; uAxis = 0; vAxis = 1; break; // t=+1
    case 2: fixedAxis = 1; fixedVal = -1.f; uAxis = 0; vAxis = 2; break; // s=-1
    case 3: fixedAxis = 1; fixedVal =  1.f; uAxis = 0; vAxis = 2; break; // s=+1
    case 4: fixedAxis = 0; fixedVal = -1.f; uAxis = 1; vAxis = 2; break; // r=-1
    case 5: fixedAxis = 0; fixedVal =  1.f; uAxis = 1; vAxis = 2; break; // r=+1
    default: break;
    }
    ...
    for (int si = 0; si < seedCount; ++si)
    {
        ...
        float3 rst0 = make_float3(0.f, 0.f, 0.f);
        if (fixedAxis == 0) rst0.x = fixedVal;
        else if (fixedAxis == 1) rst0.y = fixedVal;
        else rst0.z = fixedVal;
        float3 X0  = zidingyi::ReferenceToWorldHex(elem, rst0);
                                                                ...
                                                                int gridRes = 4)
{
    ...
    auto sample_face = [&](int faceIdx, float r, float s)->float3
    {
        ...
        switch (faceIdx)
        {
            ...
            case 1: rst = make_float3(r, s, 1.0f); break;
            case 2: rst = make_float3(r, -1.0f, s); break;
            case 3: rst = make_float3(r, 1.0f, s); break;
            case 4: rst = make_float3(-1.0f, r, s); break;
            default: rst = make_float3(1.0f, r, s); break;
        }
